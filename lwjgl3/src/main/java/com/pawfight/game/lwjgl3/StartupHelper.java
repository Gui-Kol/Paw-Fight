...
// Helper de startup LWJGL3 por SO: previne crash na extração de DLLs no Windows, reinicia a JVM com -XstartOnFirstThread no macOS e com __GL_THREADED_OPTIMIZATIONS=0 no Linux NVIDIA. Autor: damios (baseado em post de kappa).
public class StartupHelper {
	...
	// Só deve ser chamado no Linux: retorna true se houver drivers NVIDIA presentes
	...
	// Aplica os workarounds de SO; variáveis de ambiente e propriedades são copiadas ao processo filho. Retorna true se uma JVM filha foi criada
	public static boolean startNewJvmIfRequired() {
		return startNewJvmIfRequired(true);
	}

	// Igual ao anterior; inheritIO herda o I/O no processo filho e bloqueia a thread até o filho terminar
	public static boolean startNewJvmIfRequired(boolean inheritIO) {
		...
		if (osName.contains("windows")) {
			// Workaround Windows: extrai as DLLs do LWJGL3 em ProgramData (evita falha com caracteres não-ASCII no nome do usuário) e reverte as propriedades após o load
	...
	// Cria processo JVM filho no macOS ou no Linux NVIDIA (com env vars e propriedades copiadas); inheritIO bloqueia até o filho terminar
	public static boolean startNewJvm0(boolean isMac, boolean inheritIO) {
		long processID = getProcessID(isMac);
		if (!isMac) {
			// Não reinicia em Linux sem NVIDIA
			if (!isLinuxNvidia()) return false;
			// Verifica se __GL_THREADED_OPTIMIZATIONS já está desabilitado
			if ("0".equals(System.getenv("__GL_THREADED_OPTIMIZATIONS"))) return false;
		} else {
			// -XstartOnFirstThread é desnecessário em Graal native image
			if (!System.getProperty("org.graalvm.nativeimage.imagecode", "").isEmpty()) return false;

			// Verifica se já estamos na main thread (ex.: rodando via Construo)
		...
		// Impede loop infinito de JVMs filhas (estado que normalmente não deveria ocorrer)
		...
		// Assume Java 8, o mínimo para LWJGL3
